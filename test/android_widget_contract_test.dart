...
void main() {
  test('widget refresh is wired to a private broadcast receiver', () {
    final provider = File(
      'android/app/src/main/kotlin/com/example/packetstream_mobile/widget/'
      'PacketStreamWidgetProvider.kt',
    ).readAsStringSync();

    expect(provider, contains('val pendingRefreshIntent = PendingIntent.getBroadcast('));
    expect(
      provider,
      contains('Intent(context, PacketStreamWidgetRefreshReceiver::class.java)'),
    );
    expect(provider, isNot(contains('val pendingRefreshIntent = PendingIntent.getActivity(')));
  });

  test('the manual refresh receiver is not exported to other apps', () {
    final manifest = File('android/app/src/main/AndroidManifest.xml').readAsStringSync();

    final receiverIndex = manifest.indexOf('.widget.PacketStreamWidgetRefreshReceiver');
  ...
  test('manual refresh is handed to WorkManager instead of a short broadcast task', () {
    final receiver = File(
      'android/app/src/main/kotlin/com/example/packetstream_mobile/widget/'
      'PacketStreamWidgetRefreshReceiver.kt',
    ).readAsStringSync();

    expect(receiver, contains('OneTimeWorkRequestBuilder<PacketStreamWidgetWorker>()'));
